...
// Units antigas do modelo "exibição agendada" — removidas no uninstall/reinstall.
const LEGACY_UNIT_NAMES = [
...
/** Gera o conteúdo das units a partir da config (dashboard sempre ativa). */
export function renderUnits(config) {
  ...
  units['servboard-web.service'] = ini('Unit', {
    Description: 'servBoard — dashboard web (sempre ativa; atualiza os slots sozinha)',
    ...
    Restart: 'always',
    RestartSec: '3',
  }) + ini('Install', { WantedBy: 'default.target' });
...
export async function installUnits(config, { dryRun = false, sourceFile, logger = createLogger('install') } = {}) {
  ...
  // limpa units do modelo antigo (exibição agendada), se existirem
  const removedLegacy = [];
  for (const name of LEGACY_UNIT_NAMES) {
    const dest = join(USER_UNIT_DIR, name);
    if (existsSync(dest)) {
      await systemctlUser(['disable', '--now', name]);
      await rm(dest);
      removedLegacy.push(name);
      logger.info(`removida unit antiga ${name}`);
    }
  }
  ...
  const enable = await systemctlUser(['enable', '--now', 'servboard-web.service']);

  lines.push('servBoard instalado (systemd --user) — dashboard sempre ativa.');
  lines.push(`  units: ${USER_UNIT_DIR}`);
  lines.push(`  config: ${sourceFile}`);
  lines.push(`  refresh dos slots: a cada ${config.refresh.everyMinutes} min (loop interno)`);
  if (removedLegacy.length) lines.push(`  removidas units antigas: ${removedLegacy.join(', ')}`);
  lines.push(`  ${enableLinger}`);
  lines.push(`  daemon-reload: ${reload.ok ? 'ok' : reload.error}`);
  lines.push(`  servboard-web.service: ${enable.ok ? 'habilitado e no ar' : enable.error}`);
  lines.push('');
  lines.push('Verifique:  systemctl --user status servboard-web.service servboard-kiosk.service');
  lines.push('Logs:       journalctl --user -u servboard-web.service -f');
...
export async function uninstallUnits({ dryRun = false, logger = createLogger('install') } = {}) {
  const lines = [];
  if (!dryRun) {
    await systemctlUser(['disable', '--now', 'servboard-web.service']);
    await systemctlUser(['stop', 'servboard-kiosk.service']);
  }
  for (const name of [...UNIT_NAMES, ...LEGACY_UNIT_NAMES]) {
    const dest = join(USER_UNIT_DIR, name);
    if (existsSync(dest)) {
